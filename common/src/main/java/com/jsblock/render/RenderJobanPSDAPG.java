...
public class RenderJobanPSDAPG<T extends BlockPSDAPGDoorBase.TileEntityPSDAPGDoorBase> extends BlockEntityRendererMapper<T> implements IGui, IBlock {
    ...
    public void render(T entity, float tickDelta, PoseStack matrices, MultiBufferSource vertexConsumers, int light, int overlay) {
        ...
        switch (type) {
            case 0:
                final VertexConsumer vertexConsumerAPGDoor = vertexConsumers.getBuffer(MoreRenderLayers.getExterior(new ResourceLocation(String.format("jsblock:textures/block/psdapg/drlapg/apg_door_%s_%s.png", half ? "top" : "bottom", side ? "right" : "left"))));
                (half ? MODEL_APG_TOP : MODEL_APG_BOTTOM).renderToBuffer(matrices, vertexConsumerAPGDoor, light, overlay, 1, 1, 1, 1);
                if (half && !unlocked) {
                    final VertexConsumer vertexConsumerDoorLocked = vertexConsumers.getBuffer(MoreRenderLayers.getExterior(new ResourceLocation("mtr:textures/block/sign/door_not_in_use.png")));
